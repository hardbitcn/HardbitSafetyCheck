...
public class Wallet implements Serializable, BlockChainListener, PeerFilterProvider {
    ...
    /*The receive function for hardware wallet
     * 
     */  
  private void receive1(Transaction tx ) throws VerificationException {
	  // Runs in a peer thread.
	  checkState(lock.isHeldByCurrentThread());
	  BigInteger prevBalance = getBalance();
	  Sha256Hash txHash = tx.getHash();
	  ...
	  log.info(  bitcoinValueToFriendlyString(valueDifference), tx.getHashAsString() );
	  ...
	  // least we need to ensure we're manipulating the canonical object rather than a duplicate.
	  	...
	  	// If a pending transaction is confirmed, change the state
    ...
    public KeyParameter encrypt(CharSequence password) {
    	// Log.println(3, "alan", "encryptwallet1");
        checkNotNull(password);
       // Log.println(3, "alan", "encryptwallet2");
        checkArgument(password.length() > 0);
      //  Log.println(3, "alan", "encryptwallet3");
        KeyCrypter scrypt = new KeyCrypterScrypt();
     //   Log.println(3, "alan", "encryptwallet4");
        KeyParameter derivedKey = scrypt.deriveKey(password);
     //   Log.println(3, "alan", "encryptwallet5");
        encrypt(scrypt, derivedKey);
     //   Log.println(3, "alan", "encryptwallet6");
        return derivedKey;
    }
