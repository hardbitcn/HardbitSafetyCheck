...
public class Transaction extends ChildMessage implements Serializable {
    ...
    public synchronized void signInputs(SigHash hashType, Wallet wallet, @Nullable KeyParameter aesKey) throws ScriptException {
     ...
     // This is a temporary function to test the dividing of transaction
    public synchronized void signraw(SigHash hashType, ECKey key, @Nullable KeyParameter aesKey) throws ScriptException {
        checkState(inputs.size() > 0);
        checkState(outputs.size() > 0);
        ...
        for (int i = 0; i < inputs.size(); i++) {
            ...
            // The anyoneCanPay feature isn't used at the moment.
            boolean anyoneCanPay = false;
            byte[] connectedPubKeyScript =key.getPubKey()  ;          		//input.getOutpoint().getConnectedPubKeyScript();
            if (key.hasPrivKey() || key.isEncrypted()) {
                signatures[i] = calculateSignature(i, key, aesKey, connectedPubKeyScript, hashType, anyoneCanPay);
            } else {
                // Create a dummy signature to ensure the transaction is of the correct size when we try to ensure
                // the right fee-per-kb is attached. If the wallet doesn't have the privkey, the user is assumed to
                // be doing something special and that they will replace the dummy signature with a real one later.
            	throw new RuntimeException("no privatekey " );
                //signatures[i] = TransactionSignature.dummy();
            }
        }
        ...
        for (int i = 0; i < inputs.size(); i++) {
            ...
            //final TransactionOutput connectedOutput = input.getOutpoint().getConnectedOutput();
            //checkNotNull(connectedOutput);  // Quiet static analysis: is never null here but cannot be statically proven
            Script scriptPubKey = ScriptBuilder.createOutputScript(key.toAddress(Constants.NETWORK_PARAMETERS));
            //connectedOutput.getScriptPubKey();
            if (scriptPubKey.isSentToAddress()) {
                input.setScriptSig(ScriptBuilder.createInputScript(signatures[i], key));
            } else if (scriptPubKey.isSentToRawPubKey()) {
                input.setScriptSig(ScriptBuilder.createInputScript(signatures[i]));
            } else {
                // Should be unreachable - if we don't recognize the type of script we're trying to sign for, we should
    ...
    public byte[] verify1(ECKey key) throws VerificationException {
        maybeParse();
        ...
        if (this.getMessageSize() > Block.MAX_BLOCK_SIZE)
            throw new VerificationException("Transaction larger than MAX_BLOCK_SIZE");
        BigInteger inputtotalvalue= BigInteger.ZERO;
        BigInteger outputtotalvalue= BigInteger.ZERO;
        BigInteger transactionfee= BigInteger.ZERO; 
        byte[] inputscrypt=new byte[0];
        for (TransactionInput input : inputs) {
            if (input.getValue().compareTo(BigInteger.ZERO) < 0)
                throw new VerificationException("Transaction input negative");
            inputtotalvalue = inputtotalvalue.add(input.getValue());
            inputscrypt=input.getScriptBytes();
            if (inputscrypt.length==138)           {
            	
            	}
            else{
            	//throw new VerificationException("Signature script length error");
            }
            // if (key.verify(hash.getBytes(),inputs.get(0).getScriptSig().getChunks().get(0).data)){};
        ...
        for (TransactionOutput output : outputs) {
            if (output.getValue().compareTo(BigInteger.ZERO) < 0)
                throw new VerificationException("Transaction output negative");
            outputtotalvalue = outputtotalvalue.add(output.getValue());
        }
        transactionfee=inputtotalvalue.subtract(outputtotalvalue);
        if (transactionfee.compareTo(REFERENCE_DEFAULT_MIN_TX_FEE)!=0)        {
            throw new VerificationException("fee is wrong"+transactionfee.toString());
        } 
         return inputscrypt;
