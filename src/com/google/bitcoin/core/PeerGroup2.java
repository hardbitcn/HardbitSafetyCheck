 ...
 */
public class PeerGroup2 extends AbstractExecutionThreadService implements TransactionBroadcaster {
    private static final int DEFAULT_CONNECTIONS = 4;

    private static final Logger log = LoggerFactory.getLogger(PeerGroup2.class);
    ...
    public PeerGroup2(NetworkParameters params) {
        this(params, null);
    }
     ...
     */
    public PeerGroup2(NetworkParameters params, @Nullable AbstractBlockChain chain) {
     ...
     */
    public PeerGroup2(NetworkParameters params, @Nullable AbstractBlockChain chain, ClientConnectionManager connectionManager) {
     ...
     * and then calling {@link PeerGroup2#setVersionMessage(VersionMessage)} on the result of that. See the docs for
    ...
    /**
     * Same as {@link PeerGroup2#addEventListener(PeerEventListener, java.util.concurrent.Executor)} but defaults
     ...
     *   <li>Set the fast catchup time using {@link PeerGroup2#setFastCatchupTimeSecs(long)}, to optimize chain
     *       download.</li>
     * </ol>
     *
     ...
     * {@link PeerGroup2#removeWallet(Wallet)} on it if you wish to keep the Wallet but lose the PeerGroup.</p>
     */
     ...
     * {@link PeerGroup2#addWallet(Wallet)} instead.</p>
     *
        ...
        pingRunnable[0] = new Runnable() {
            ...
            public void run() {
                ...
                final TimerTask task = new TimerTask() {
                    ...
                    public void run() {
                        try {
                            if (!peers.contains(peer) || !PeerGroup2.this.isRunning())
     ...
     * cannot contain any interesting transactions. If you use {@link PeerGroup2#addWallet(Wallet)} this just returns
     ...
     */
    public ListenableFuture<PeerGroup2> waitForPeers(final int numPeers) {
        lock.lock();
        try {
        ...
        final SettableFuture<PeerGroup2> future = SettableFuture.create();
     ...
     * enough, {@link PeerGroup2#broadcastTransaction(Transaction)} will wait until the minimum number is reached so
     * propagation across the network can be observed. If no value has been set using
     * {@link PeerGroup2#setMinBroadcastConnections(int)} a default of half of whatever
    ...
    /**
     * Calls {@link PeerGroup2#broadcastTransaction(Transaction,int)} with getMinBroadcastConnections() as the number
     ...
     * times are available via {@link com.google.bitcoin.core.Peer#getLastPingTime()} but it increases load on the
     * remote node. It defaults to {@link PeerGroup2#DEFAULT_PING_INTERVAL_MSEC}.
